...
Original file is located at
    https://colab.research.google.com/drive/1Tsr4gCrvm5kPcLxFQBp33m3Ke2iF00tu
...
#TESTing from above to see where the clusters are
from scipy.spatial.distance import cdist

n_clusters = 10
random_state = 10  # makes sure you get the same results each time. Random state is about making it predictable so we get the results.
...
def fit_kmeans(X, n_clusters, random_state):
  ...
  model = KMeans(n_clusters=n_clusters,
                 init='k-means++', #K-means++ is the standard K-means algorithm coupled with a smarter initialization of the centroids.
                 n_init=10, #n_init states how many different sets of random points the algorithm should use.
                 tol=1e-04, #tol used to adjust intended tolerance convergence when inertia is calculated after each iteration
...
# Reference Kmeans definitions:
# 1) https://holypython.com/k-means/k-means-optimization-parameters/
...
## Print histogram of clusters by colors for better visualization.
df1 = df_colors[df_colors.cluster==0]
df2 = df_colors[df_colors.cluster==1]
df3 = df_colors[df_colors.cluster==2]
df4 = df_colors[df_colors.cluster==3]
df5 = df_colors[df_colors.cluster==4]
df6 = df_colors[df_colors.cluster==5]
df7 = df_colors[df_colors.cluster==6]
df8 = df_colors[df_colors.cluster==7]
df9 = df_colors[df_colors.cluster==8]
df10 = df_colors[df_colors.cluster==9]

#pd.DataFrame.hist(df1.cluster)
plt.hist(df1['cluster'])
plt.hist(df2['cluster'])
plt.hist(df3['cluster'])
plt.hist(df4['cluster'])
plt.hist(df5['cluster'])
plt.hist(df6['cluster'])
plt.hist(df7['cluster'])
plt.hist(df8['cluster'])
plt.hist(df9['cluster'])
plt.hist(df10['cluster'])
...
def guess_color(centroids, color_dict, test_color):
  ...
  # Convert test_color into np.array
  testColorNp = np.array(test_color)

  # Have testColorNp be the same shape as centroids
  testColorNew = np.array([testColorNp for i in range(10)])

  # Get sum of each row of the square of all differences
  diffSq = np.square(centroids - testColorNew)
  diffSumSq = np.sum(diffSq, axis=1)

  # Euclidean distances
  euclideanDist = np.sqrt(diffSumSq)

  # Get the smallest distance
  minEucDiff = np.min(euclideanDist)

  # Get the index
  index = np.where(euclideanDist == minEucDiff)

  color_name = color_dict[index[0][0]]
  return color_name
...
def predict_color(model, color_dict, test_color):
  ## YOUR CODE HERE ##
  km = model.predict(np.array([test_color]))
  color_name = color_dict[km[0]]
...
def make_test_colors(n=3):
  ...
  for rr in range(n+1):
    for gg in range(n+1):
      for bb in range(n+1):
        red = int(rr/n*255) #rr/n everything * 255, max should be 255
        green = int(gg/n*255)
        blue = int(bb/n*255)
...
for ii,cc in enumerate(test_colors):
  ## do something clever
  color_name = guess_color(centroids, color_dict, cc)
  colorNamesList.append(color_name)
...
for i in range(nrows):
  for j in range(ncols):
    index = i*nrows+j
    axs[i,j].imshow([[test_colors[index]]])
    axs[i,j].set_title([colorNamesList[index]])
    plt.axis('off')

#Visualize test_colors as individual circles, like Exercise 7.
test_colors = make_test_colors()
...
The takeaway from this week's material and homework is that clustering can be tricky and doesn't make perfect predictions for colors. I found that the 'Examine results for clustering' portion difficult to identify these colors and their gradients. Furthermore, in Exercise 8, I attempted to display the colors and see if they were correct and it's not so straight forward and there were a number of incorrect color predictions. It seems no matter what you cluster, there may be one group that is clustering correctly and makes sense, but another group of clusters that may not be so well defined. It seems it is difficult to cluster all correctly or perfectly and I think that's because when we're quantizing data, it falls onto a spectrum of categories or groups and it is difficult to have precise correct clustering.

I also noticed that kmeans clustering can offer different results when ran on different algoritms with minor adjustments (for example, I tested initializing and not, using different color names, etc. and it would get different cluster results and centrioids). This makes the clustering results inconsistent.

Nevertheless, I understand the inherent value of Kmeans clustering for offering simple clustering that's flexible, can be applied to large, complex datasets, and that's easily explainable.
